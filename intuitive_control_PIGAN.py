...
def get_control(input_name, args):
    ...
    # Load facial expressions from mat file
    ...
    # Get intensity scale factor (1.0 to 2.0)
    intensity_scale = args.intensity * 0.25 + 0.75  # Maps 1->1.0, 3->1.5, 5->2.0
    ...
    if args.happy:
        ...
        control_dict['expression_sadness'] =  \
           - (control_dict['expression_eyebrow']) * 0.9 * intensity_scale - \
            (control_dict['expression_eyes']) * 0.15 * intensity_scale - \
            (control_dict['expression_mouth']) * 0.8 * intensity_scale
        #- control_dict['expression_center']
        selected_expression = 'expression_sadness'
        emotion_name = "Sadness"
    
    elif args.angry:
        # Adjusted coefficients for a better angry expression:
        control_dict['expression_anger'] =                                \
             (control_dict['expression_eyebrow']) * 0.2 * intensity_scale \
           + (control_dict['expression_eyes']) * 0.2 * intensity_scale   \
           + (control_dict['expression_mouth']) * 0.2 * intensity_scale
        ...
        control_dict['expression_fear'] = control_dict['expression_center'] + \
            ...
            (control_dict['expression_eyebrow'] - control_dict['expression_center']) * 0.8 * intensity_scale + \
            (control_dict['expression_mouth'] - control_dict['expression_center']) * 0.7 * intensity_scale
    ...
    # Simplified animation sequence: directly from input image to emotion
